...
angular.module('fsApp')
  .factory('fsConfig', function($http, $q, ppSyncService) {
    var config, configuration;

    localStorage.removeItem('fsConfig');

    function loadConfigFromLocalStorage() {
      configuration = JSON.parse(localStorage.getItem('fsConfig'));
      return configuration;
    }

    function saveConfigtoLocalStorage(config) {
      localStorage.setItem('fsConfig', JSON.stringify(config));
    }
...
angular.module('fsApp')
  .factory('fsUser', function() {
    ...
    var initUser = function() {
      if (!localStorage.getItem('fsUser')) {
        currentUser = userAttributes;
      } else {
        currentUser = JSON.parse(localStorage.getItem('fsUser'));
      }
    };
    initUser();
    ...
    function changeUser(user) {
      ...
      localStorage.setItem('fsUser', JSON.stringify(user));
    }
        ...
        link: function($scope, element, attrs) {
          ...
          function updateCSS() {
            if (userRole && accessLevel) {
              if (!fsUser.authorize(accessLevel, userRole)) {
...
angular.module('fsApp')
  .factory('fsPostHelper', function($q, fsGeolocation) {
    ...
    return {
      ...
      createPostObject: function(content, user) {
        ...
        tempPost.coords = fsGeolocation.getCurrentCoords();
        tempPost.user = {
          id: user.id,
          name: user.name
        };
      ...
      createImageObject: function(content, user) {
        ...
        tempImage.coords = fsGeolocation.getCurrentCoords();
        tempImage.user = {
          id: user.id,
          name: user.name
        };
      ...
      createLikeObject: function(user, posting) {
        ...
        tempLike.coords = fsGeolocation.getCurrentCoords();
        tempLike.user = {
          id: user.id,
          name: user.name
        };
      ...
      createCommentObject: function(content, user, posting) {
        ...
        tempComment.coords = fsGeolocation.getCurrentCoords();
        tempComment.user = {
          id: user.id,
          name: user.name
        };
...
angular.module('fsApp')
  .factory('fsGeolocation', function() {
    ...
    var saveCurrentLocationtoLocalStorage = function() {
      localStorage.setItem('fsLocation', JSON.stringify(coords));
    };
    var loadCurrentPositionFromLocalStorage = function() {
      return JSON.parse(localStorage.getItem('fsLocation'));
    };

    var saveCurrentMapPositionToLocalStorage = function(position) {
      localStorage.setItem('fsMapPosition', JSON.stringify(position));
    };
    var loadCurrentMapPositionFromLocalStorage = function() {
      return JSON.parse(localStorage.getItem('fsMapPosition'));
    };
...
angular.module('fsApp').factory('fsID', function($cookies) {
    return {
        init : function () {
